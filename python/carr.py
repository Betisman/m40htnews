...
def carruselear():
        ...
        for matchid in matchids:
                ...
                try:
                        ...
                        if awayteam.startswith('Anto'):
                                awayteam = 'ant'
                        hometeam =hometeam.replace('F.C.Malpika', 'fcm')
                        if hometeam.find('Betisman') > -1:
                                hometeam = 'RBB'
                        awayteam =awayteam.replace('Basullo', 'Bas')
                        # fin parche ##############################
                        # MOLINORRRRRRRRRRRRRRRRRR
												cadapartido = marcador.createElement("partido")
												local = cadapartido.createElement("equipolocal")
												local.appendChild(local.createTextNode(hometeam))
												cadapartido.appendChild(local)
												visitante = cadapartido.createElement("equipovisitante")
												visitante.appendChild(visitante.createTextNode(awayteam))
												cadapartido.appendChild(visitante)
												goleslocal = cadapartido.createElement("goleslocal")
												goleslocal.appendChild(goleslocal.createTextNode(homegoals))
												cadapartido.appendChild(goleslocal)
												golesvisitantes = cadapartido.createElement("golesvisitante")
												golesvisitante.appendChild(golesvisitante.createTextNode(hometeam))
				...
				print marcador.toprettyxml()
				afichero(str(marcador.toprettyxml()), pathXmls+'marcador.xml')
        print strResultados
